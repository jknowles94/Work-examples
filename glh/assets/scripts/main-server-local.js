...
var glh = (function(){
	...
	// var subnav = (function(){
	// 	var $widget = $('[data-widget="subnav"]');
	// 	var $fixedMenu = $widget.clone();
		
	// 	var createFixedSubnav = function(){
	// 		$fixedMenu.css({
	// 			position: 'fixed',
	// 			top: '0',
	// 			left: '0',
	...
	// 	var navVisibility = function(){
	// 		var position = $widget.position().top;
	...
	// 				createFixedSubnav();
	// 				navVisibility();
	// 				$(window).on('scroll', navVisibility);
	// 			}
	// 		}
	// 	}
	// }());
		...
		var infoBoxContent = function(content){
			...
			if(content.name){
				...
				return ('<div class="infoBox">'
					...
					+			'<p class=""><a target="_blank" href="'+content.url+'" class="btn-arrow">Go to Hotel Website</a></p>'
					+		'</div>'
					+ '</div>');
			}
			return;
		...
		var loadMap = function(){
		  ...
		  map.setMapTypeId('map_style');
			...
			if (typeof markersData!== 'undefined'){
				...
				downloadUrl('/locationmap/', function(data){
	...
	return {
		init : function(){
			...
			//Sub navigation used on products page

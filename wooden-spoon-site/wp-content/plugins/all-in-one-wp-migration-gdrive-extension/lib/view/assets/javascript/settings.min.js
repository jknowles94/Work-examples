...
jQuery(document).ready(function ($) {
	...
	if (ai1wmge_settings.token) {
		...
		.done(function (result) {
			$('#ai1wmge-gdrive-details').html('Logged in as <strong>' + result.name + '<strong>');
			$('#ai1wmge-gdrive-size').html('Google Drive space <strong>' + result.used + '</strong> of <strong>' + result.total + '</strong>');
			$('#ai1wmge-gdrive-progress-bar').width((result.progress < 10 ? '10%' : result.progress + '%')).text(result.progress + '%');
		});
	}
});

...
class ServMaskGdriveCurl {
	...
	public function __construct() {
		...
		// Enable SSL support
		$this->setOption(CURLOPT_CAINFO, dirname(__FILE__) . '/../certs/cacerts.pem');
	...
	public function makeRequest() {
		...
		if ($response === false) {
			throw new Exception(sprintf('Unable to connect to Google Drive. Error code: %d', curl_errno($this->handler)));
		}
		...
		switch ($code) {
			...
			case 503:
				throw new Exception('Serivce is not available.');
		}
